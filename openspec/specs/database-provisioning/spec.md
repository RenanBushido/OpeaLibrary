...
Ensures the API's databases (Postgres for writes, MongoDB for reads) are correctly reachable and provisioned with their required schema/collections, whether the API runs as a host process during local development or as a container on the Docker Compose network — so `docker compose up` plus an API start reliably yields a working, schema-ready system with no manual database setup steps.
