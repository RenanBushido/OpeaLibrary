# crosscutting-unit-tests Specification

## Purpose
Automated xUnit test suite validating the CrossCutting layer's dependency-injection registration (`ApplicationExtensions` and `InfrastructureExtensions`) in `src/CrossCutting`, using an in-test `IConfiguration` with placeholder connection strings so no external Postgres or MongoDB connection is required. 100% line and branch coverage of `src/CrossCutting/` is the quality bar for this suite, with one documented exception (see the 100% Code Coverage requirement).
